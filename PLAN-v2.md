...
| # | 功能 | 关键硬约束 |
|---|------|-----------|
| 1 | 多格式解析 | PDF/MD/TXT 必须，DOCX 建议；逐页解析；输出统一 JSON schema（textbook_id, filename, chapters[]） |
| 2 | 单本知识图谱构建 | 每章 LLM 抽取节点；节点 schema：`{id,name,definition,category,chapter,page}`；关系类型 ≥ 3 种（prerequisite/parallel/contains/applies_to） |
| 3 | 图谱交互可视化 | 点击节点查详情；节点大小/颜色映射出现频次；不同教材不同色；缩放+拖拽 |
| 4 | 跨教材整合（核心难点） | 语义对齐至少 1 种（推荐 embedding+LLM）；输出决策列表（merge/keep/remove + reason + confidence）；**压缩比 ≤ 30%**；前端展示压缩比 |
| 5 | RAG 问答 | chunk 500–800 字 + 50–100 字 overlap；元数据（教材/章节/页码）；向量库（FAISS/ChromaDB）；top-5 检索；**回答必须引用原文 [教材, 第X章, 第X页]**；找不到时回复"当前知识库中未找到相关信息" |
| 6 | Agent 架构说明文档 | `docs/Agent架构说明.md`，含架构总览/设计决策论证/数据流/取舍权衡；评 D 维度 20 分核心 |
| 7 | 多轮对话迭代 | 教师能用自然语言修改整合决策；对话历史持久化；图谱实时更新 |
| 8 | Web SPA 界面 | 单页应用；建议布局：左教材管理 / 中图谱 / 右功能面板；1920×1080 正常显示 |
| 9 | 整合报告 | `report/整合报告.md`，含整合概览/决策摘要/图谱统计/3-5 典型案例/教学完整性说明 |
| 10 | 开发文档 | README + 需求分析 + 系统设计 + Agent 架构说明 |
